...
string_type trim_front(const string_type& s) {
    auto it = s.begin();
...
template <typename string_type>
string_type trim_back(const string_type& s) {
    auto rit = s.rbegin();
    while (rit != s.rend() && std::isspace(*rit)) {
        ++rit;
    }
    return {s.begin(), rit.base()};
}
...
public:
    ...
    void walk_children(const walk_callback& callback) {
        ...
        decltype(count) idx = 0;
        for (; idx < count; ++idx) {
            bool trim_front = false;
            bool trim_back  = false;
            if (idx > 0) {
                if (children[idx - 1].tag.trim_after) {
                    trim_front = true;
                }
            }
            if (idx + 1 < count) {
                if (children[idx + 1].tag.trim_before) {
                    trim_back = true;
                }
            }
            if (children[idx].walk(callback) != walk_control::walk) {
                break;
            }
        }
    }
    ...
    void parse(const string_type& input, context_internal<string_type>& ctx, component<string_type>& root_component, string_type& error_message) const {
        ...
        for (string_size_type input_position = 0; input_position != input_size;) {
                ...
                for (const auto& whitespace_text : whitespace) {
                    if (input.compare(input_position, whitespace_text.size(), whitespace_text) == 0) {
                        ...
                        if (!skipping_whitespace) {
                            component<string_type> comp{whitespace_text, input_position};
                            comp.is_whitespace = true;
                            sections.back()->children.push_back(comp);
                        }
                        break;
                    }
            ...
            if (comp.tag.type != tag_type::set_delimiter) {
                ...
                if (comp.tag.trim_before) {
                    // Trim the trailing whitespace
                    while (!sections.back()->children.empty()
                           && (sections.back()->children.back().is_whitespace)) {
                        sections.back()->children.pop_back();
                    }
                }
                if (comp.tag.trim_after) {
    ...
    void parse_tag_contents(bool is_unescaped_var, const string_type& contents, mstch_tag<string_type>& tag) const {
        if (is_unescaped_var) {
            ...
            if (!inner.empty() && inner[0] == '%') {
                tag.trim_before = true;
                inner.erase(inner.begin());
                inner = trim_front(inner);
            }
            if (!inner.empty() && inner.back() == '%') {
                tag.trim_after = true;
                inner.pop_back();
                inner = trim_back(inner);
            }
            if (inner.empty()) {
                // May have trim markers, but is empty otherwise
